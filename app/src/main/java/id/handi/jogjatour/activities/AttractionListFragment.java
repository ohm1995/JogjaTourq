...
public class AttractionListFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mImageSize = getResources().getDimensionPixelSize(Integer.parseInt("120dp"))
                * konstantaGmaps.IMAGE_ANIM_MULTIPLIER;

        mLatestLocation = Utils.getLocation(getActivity());
        List<atraksi> attractions = loadAttractionsFromLocation(mLatestLocation);
    ...
    private static List<atraksi> loadAttractionsFromLocation(final LatLng curLatLng) {
        String closestCity = atraksiwisata.getClosestCity(curLatLng);
        if (closestCity != null) {
            List<atraksi> attractions = ATTRACTIONS.get(closestCity);
            if (curLatLng != null) {
                Collections.sort(attractions,
                        new Comparator<atraksi>() {
                            @Override
                            public int compare(atraksi lhs, atraksi rhs) {
    ...
    private class AttractionAdapter extends RecyclerView.Adapter<ViewHolder>
        ...
        public List<atraksi> mAttractionList;
        private Context mContext;

        public AttractionAdapter(Context context, List<atraksi> attractions) {
            super();
            mContext = context;
        ...
        public void onBindViewHolder(ViewHolder holder, int position) {
            atraksi attraction = mAttractionList.get(position);
